...
"""
Helper to train models with teachermodel.
...
class Trainer:
    ...
    def update(self, images, targets, models = None, graphs=None, embeddings=None, param_groups_map=None, ghn=None, is_imagenet=False):
        ...
        try:
            with torch.cuda.amp.autocast(enabled=self.amp):
                ...
                with torch.no_grad():
                    # teacher_logits = self.teachermodel(images)
                    teacher_label = get_teacher_label(self.teachermodel, images)
                    
                for model in models:
                    try:
                        
                        out = model(images)
                        student_label = model.trans_forward(images)
                    except:
                        print(model)
                        raise
                    y = out[0] if isinstance(out, tuple) else out
                    
                    temperature = 3 #默认teperature设置为3，可更改
                   
                    loss_kl = F.kl_div(
                        F.log_softmax(student_label / temperature, dim=1),
                        F.log_softmax(teacher_label / temperature, dim=1).detach(),
                        reduction='sum',
                        log_target=True
                    ) * (temperature * temperature) / y.numel()
                    
                    alpha = 0.2
                    
                    loss = alpha * loss_kl + (1 - alpha) * self.criterion(y, targets)
                    
                    if self.auxiliary:
                        loss += self.auxiliary_weight * self.criterion(out[1], targets)
                        # loss = alpha * loss_kl + (1 - alpha) * self.auxiliary_weight * self.criterion(y, targets)
