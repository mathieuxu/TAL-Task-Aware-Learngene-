...
class Trainer:
    def __init__(self,
        ...
        self.teachermodel = teachermodel.to(self.main_device)
        ...
        if ckpt is not None or (self.checkpoint_path is not None and os.path.exists(self.checkpoint_path)):
            ...
            if self.checkpoint_path is not None and os.path.exists(self.checkpoint_path):
                ...
                model.to('cpu')
                model = ghn(model, task_embedding, bn_track_running_stats=True, keep_grads=False, reduce_graph=False)  # predict params
    ...
    def _reset(self, opt, opt_args, scheduler, scheduler_args, state_dict):
        ...
        trainable_params = [param for name, param in self._model.named_parameters() if param.requires_grad]

        for param in self._model.parameters():
            param.requires_grad = True

        self._optimizer = optimizer(self._model.parameters(), **opt_args)
        # self._optimizer = optimizer(trainable_params, **opt_args)
    ...
    # def update(self, images, targets, label_map, task_embedding, models = None, graphs=None, embeddings=None, param_groups_map=None, ghn=None, is_imagenet=False):
    def update(self, images, targets, num_class, task_embedding, dataset_name, models = None, graphs=None, embeddings=None, param_groups_map=None, ghn=None, is_imagenet=False):
        # Save dataset_name as an attribute of the trainer
        ...
        try:
            with torch.cuda.amp.autocast(enabled=self.amp):
                ...
                # targets = map_labels(targets, label_map)
    ...
    def log(self, step=None):
        ...
        if step_ % self.log_interval == 0 or step_ >= self.n_batches - 1 or step_ == 1:
            ...
            # Include dataset_name in the log
