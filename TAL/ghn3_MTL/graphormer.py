...
TEN is added to process task_embedding.
...
def create_transformer(Module, Linear, GELU, ReLU, LayerNorm, Dropout, Identity, Sequential):
    ...
    # Process Task_embedding information
    class TEN(nn.Module):
        def __init__(self, input_dim, task_embedding_dim):
            super(TEN, self).__init__()

            self.fc_gamma = nn.Linear(task_embedding_dim, input_dim)
            self.fc_beta = nn.Linear(task_embedding_dim, input_dim)
        ...
        def forward(self, x, task_embedding):
            ...
            gamma = self.fc_gamma(task_embedding)
            beta = self.fc_beta(task_embedding)
    ...
    class TransformerLayer(Module):
        ...
        def forward(self, x, task_embedding, edges=None, mask=None):
            ...
            x_attn, edges = self.attn(self.TEN(self.ln1(x), task_embedding), edges, mask)
            x = x + x_attn
            ...
            return (x, task_embedding, edges, mask) if self.return_edges else (x, task_embedding)
    return locals()
